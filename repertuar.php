...
<span class="navbar-toggler-icon"></span>
  ...
  <div class="main bg-secondary text-white">
    ...
    <?php
                    $day_array = array(1=>'Pon', 'Wt', 'Śr', 'Czw', 'Pt', 'Sob', 'Ndz');
                    $sql = "SELECT * FROM movies group by title;";
                    ...
                    if ($resultCheck > 0) {
                        while ($row = mysqli_fetch_assoc($result)){
                           ...
                           print '<figure> <img src="'.$image.'" width="60% height="60%" alt="plakat"/>';
                           print '<figcaption>'.$row['title'].'</figcaption></figure>';
                           print '<div class="opis"><p>'.$row['description'].'</br></br>Czas trwania: '.$row['length'].'</p></div>';
                           
                           print '<div id="dates">';
                           foreach ($day_array as $day_value => $day_string){
                               print '<div class="day"><form>'.$day_string.'</br></br>';
                               $day = $day_value;
                               $title = $row['title'];
                               $day_query = ($con->query("select id_movie,time FROM movies where day = '$day' and title like '$title'"));
                               while ($time = $day_query->fetch_array()){
                                   print '<a href="rezerwacja.php?id_movie='.$time['id_movie'].'" title="Kliknij tutaj żeby zarezerwować"><input type="button" name="godz" value="'.$time['time'].'"/></a>';
                               }
                               print '</form></div>';
                           }
                           print '</div>';
